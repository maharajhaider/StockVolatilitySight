...
\section*{2026-04-24 (late) --- Full Colab pipeline results (multi-seed, k = 3)}

\paragraph{Run details.} Executed nb 01 $\to$ nb 07 on Colab free-tier CPU. Total wall time $\sim$95 minutes (nb 04: 35 min, nb 05: $\sim$50 min, others minor). 12 baseline LSTM checkpoints + 18 regime LSTM checkpoints + 3 HMMs (variants O, A, B) trained. Common test-date index (intersection across 5 prediction parquets): \textbf{$n = 1{,}440$ rows}, 2020-06-30 $\to$ 2026-03-24. All metrics below are mean-of-3-seeds aggregates unless noted.

\subsection*{Headline 6-way metric table (mean-of-seeds, common index, $n = 1{,}440$)}
...
Predictor & MSE & RMSE & MAE & MAPE \\
\midrule
naive (\texttt{rolling\_std\_21})        & $2.20 \times 10^{-5}$ & $0.00469$ & $0.00313$ & $34.59\%$ \\
HAR-RV                                   & $1.61 \times 10^{-5}$ & $0.00401$ & $0.00278$ & $31.26\%$ \\
...
\textbf{Three tiers visible on MAPE.} Top ($\sim 24\text{-}26\%$): A, B, H --- statistically indistinguishable. Middle ($\sim 30\text{-}31\%$): variant O ensemble, HAR-RV. Bottom ($35\%$): naive. Sentiment is the largest single feature contribution: O $\to$ A drops MAPE by 5.9 pp ($30.14 \to 24.23$). Adding VIX on top (A $\to$ B) is a wash ($24.23 \to 25.51$, within cross-seed noise).

\subsection*{Within-variant DM tests (the core research-question answer, nb 06 \S 5)}
...
\toprule
Variant & DM MSE & $p_{\text{MSE}}$ & DM MAE & $p_{\text{MAE}}$ & Verdict ($\alpha = 0.05$) \\
\midrule
O          & $+0.14$ & $0.89$  & $-1.79$ & $0.07$  & \textbf{tie} (both losses) \\
A          & $+1.46$ & $0.15$  & $+1.92$ & $0.06$  & \textbf{tie} (marginal MAE) \\
\textbf{B} & $\mathbf{+2.17}$ & $\mathbf{0.030}$ & $\mathbf{+2.25}$ & $\mathbf{0.025}$ & \textbf{ensemble wins (significant)} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Refined research finding.} Regime-splitting only yields a statistically significant gain over a baseline LSTM \textbf{when forward-looking VIX inputs are available (variant B)}. On price/volume alone (O) or with sentiment alone (A), regime-splitting does not significantly improve over the single-LSTM baseline. This refines the prior structural-limits framing --- it's not ``regime-splitting never helps''; it's ``regime-splitting only helps when the LSTM itself has rich enough inputs to make the volatile component informative.''

\textbf{Why this matters for the paper.} The original hypothesis was binary (regime-splitting beats baseline, yes/no). The refined version is interaction-shaped: \emph{the value of regime-splitting depends on what the underlying LSTMs can learn from}. Variant A's volatile LSTM has nothing additional to add over A's baseline; variant B's volatile LSTM extracts incremental signal from VIX that the regime-gate can leverage.

\subsection*{Cross-variant DM, 30 pairwise tests with BH-FDR correction (nb 07 \S 5)}

After Benjamini--Hochberg FDR correction at $\alpha = 0.05$, \textbf{9 of 30 tests reject} (sorted by raw $p$, ascending):

\begin{center}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}p{2.6cm}p{3.2cm}lrrrl@{}}
\toprule
A & B & loss & DM & raw $p$ & BH-FDR & Verdict \\
\midrule
...
\textbf{Translation.} (a) All learned variants and HAR-RV beat naive on MAE. (b) A, B, H all beat HAR-RV on MAE --- deep learning + sentiment outperforms the canonical 3-feature OLS by a measurable margin. (c) \textbf{Variant O loses to A, B, H on MAE} --- variant O is essentially tied with HAR-RV at the bottom of the learned-models tier. (d) \textbf{A vs B vs H pairwise: all ties under BH-FDR} --- the three richer-feature variants are statistically indistinguishable from each other.

\subsection*{F1 --- persistence-shift floor SURVIVES across all variants (nb 07 \S 6)}
...
\midrule
naive                    & $-21$   & $1.000$ (identity) & $0.457$ \\
HAR-RV                   & $\mathbf{-21}$ & $0.947$ & $0.494$ \\
variant O (ensemble)     & $-16$   & $0.799$ & $0.541$ \\
\textbf{variant A (ensemble)} & $-17$ & $0.835$ & $\mathbf{0.601}$ \\
...
\textbf{The persistence-shift floor is real and structural.} Deep models pull the lag from $-21$ (naive / HAR-RV) to $-16/-17$ --- a \textbf{4-5 day improvement} but it never reaches zero. The HAR-RV finding is especially clean: HAR-RV peaks at the same $-21$ as naive because its dominant $\beta_m$ predictor ($\beta = 0.494$) is exactly rolling vol over 21 days back. \textbf{HAR-RV is essentially ``naive with a bias correction.''} Lag-0 correlation also improves: $0.46$ (naive) $\to$ $0.60$ (variant A) --- deep models extract genuinely more same-day information than persistence, but only $\sim 30\%$ more.

\subsection*{F2 --- volatile-LSTM degeneracy is variant-specific and seed-dependent (nb 07 \S 7)}

Per-(variant $\times$ seed) prediction std for the volatile-regime LSTM (target std on common index $= 0.0044$):

\begin{center}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}lrrrl@{}}
\toprule
Variant $\times$ seed & Volatile-LSTM std & Range & std / target & Note \\
\midrule
\textbf{A} seed 42 & $\mathbf{0.732}$  & $2.72$  & $165\times$ & wildly non-stationary \\
A seed 43          & $0.259$           & $2.14$  & $59\times$  &  \\
\textbf{A} seed 44 & $\mathbf{0.859}$  & $2.78$  & $195\times$ & wildly non-stationary \\
\textbf{B} seed 42 & $0.056$           & $0.44$  & $12.6\times$ & \\
\textbf{B} seed 43 & $\mathbf{0.0004}$ & $0.001$ & $0.10\times$ & collapsed to constant \\
B seed 44          & $0.007$           & $0.034$ & $1.5\times$  &  \\
\textbf{O} seed 42 & $0.0031$          & $0.025$ & $0.71\times$ & stable / functional \\
\textbf{O} seed 43 & $0.0032$          & $0.021$ & $0.72\times$ & stable / functional \\
\textbf{O} seed 44 & $0.0038$          & $0.027$ & $0.86\times$ & stable / functional \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Two F2 failure modes are present, both seed-dependent for variants A and B.} Variant A's volatile LSTM blows up to wildly non-stationary outputs (pred std 60-200$\times$ target std --- implausible vol predictions in the $[-1, +2]$ range). Variant B's volatile LSTM mostly collapses to near-constant output (one seed has std $4 \times 10^{-4}$, two orders of magnitude below target). Both are pathologies; the soft-prob ensemble gating limits the damage because \texttt{p\_volatile} is small on most days.

\textbf{Variant O is the exception that explains the rule.} O's volatile LSTM has prediction std $0.003\text{-}0.004$ across all 3 seeds --- comparable to target std $0.0044$, behaving like a real predictor. Why? \textbf{Variant O's HMM labels $\sim 60\%$ of test days as strictly volatile} ($p_{\text{volatile}} > 0.8$) --- the regime-stratified breakdown in nb 06 \S 7 shows O has 868 strict-volatile test days vs 27-32 for variants A and B ($\sim 30\times$ more). O's HMM is sentiment-free and labels regimes much more aggressively.

This means F4 (data scarcity) is not a property of training-window size in absolute terms --- it's a property of \textbf{how aggressively the HMM labels volatile regimes}. With sentiment in the HMM (variants A, B), the HMM is conservative $\to$ $\sim 180$ volatile-majority training windows $\to$ F2 emerges. Without sentiment (variant O), the HMM is liberal $\to$ $\sim 2{,}200$ volatile-majority training windows $\to$ F2 disappears.

\textbf{This is a meaningful refinement of F4} --- data scarcity is HMM-dependent, not a fundamental property of price/volume daily data.

\subsection*{Cross-seed variance disclosure (nb 07 \S 4c)}
...
\textbf{variant B (ensemble)} & $\mathbf{13.5}$ & $\mathbf{6.7}$ & $\mathbf{6.3}$ & $6.4$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Variant B is the noisiest} --- point-estimate MSE $1.29 \times 10^{-5}$ has a $\pm 13.5\%$ cross-seed swing. Direct evidence that variant B's seed-dependent F2 failure mode propagates into the headline metrics. This is why the within-variant DM verdict for B (ensemble wins) is harder to interpret than for the more-stable variants --- the ``wins'' margin needs to overcome larger seed-induced noise. With $k = 5$ seeds, B's CV would tighten and we could trust the verdict more.

\textbf{Variant O is the most stable} ($3.2\%$ MSE CV) --- consistent with its no-F2 status above. A and H are intermediate ($5\text{-}7\%$).

\subsection*{Refined defensible claims for the paper}

\begin{enumerate}[leftmargin=*]
  \item \emph{``Adding AAII sentiment to the LSTM features (variant O $\to$ variant A) reduces test MSE by $9\%$ and MAPE by $6$ percentage points ($30.14 \to 24.23\%$), and the improvement is statistically significant (DM MAE BH-FDR $p < 0.001$).''} --- strongest single-feature finding.

  \item \emph{``Adding VIX-family inputs on top of sentiment (variant A $\to$ variant B) does not significantly improve point-estimate metrics on the cross-variant comparison (BH-FDR $p > 0.7$ on MAE), even though variant B is the only pipeline where the regime-split ensemble significantly beats its single-LSTM baseline (within-variant DM, MSE $p = 0.030$, MAE $p = 0.025$).''} --- interaction-shaped finding.

  \item \emph{``Regime-splitting helps in a feature-dependent way: ties for variants O and A; significant for variant B. The structural-limits claim refines from `regime-splitting never helps' to `regime-splitting helps only when the LSTM itself has access to forward-looking implied-volatility signal.'\,''} --- refined headline.

  \item \emph{``All learned variants (A, B, H) beat the canonical HAR-RV econometric benchmark on MAE (BH-FDR $p \le 0.001$), confirming that deep models with sentiment extract incremental signal beyond what HAR-RV's three-feature OLS can capture. Variant O ties with HAR-RV --- without sentiment, the deep model offers no benefit over the simple OLS.''} --- comparative benchmark claim.

  \item \emph{``The persistence-shift floor (F1) survives all six predictors. Naive and HAR-RV peak at lag $-21$; learned variants peak at $-16$ to $-17$. The shift narrows by 4-5 days but never reaches zero. The phenomenon is structural --- a property of daily-frequency $\times$ 21-day-forward target --- not feature- or architecture-dependent.''} --- F1 paper claim, now confirmed across HAR-RV too.

  \item \emph{``F2 (volatile-LSTM degeneracy) appears in two distinct seed-dependent failure modes (variant A: wildly non-stationary output, std $165\text{-}195\times$ target; variant B: collapsed near-constant, std $0.10\times$ target on the most affected seed). Variant O is the exception, with stable volatile-LSTM behaviour (std $\sim 0.7\times$ target) --- explained by O's HMM labelling $60\%$ of days as volatile vs $2\%$ for A/B, giving $\sim 30\times$ more volatile-majority training windows.''} --- F2 + F4 refinement.

  \item \emph{``Multi-seed ($k = 3$) reporting is essential at this scale. Variant B's MSE has $13.5\%$ cross-seed CV, so single-seed point estimates can swing by $\sim 25\%$ between runs. Mean $\pm$ std reporting protects against picking lucky/unlucky seeds for headline numbers.''} --- methodology disclosure.
\end{enumerate}
